...
def read_bands(record: dict, bands: List[str], plot: Tuple[Any, ...], width: int, height: int) -> dict:
    ...
    for band in bands:
        ...
        while(cnt>0):
            try:
                my_href = StacRecord.asset_href(record, band)
                with rio.open(StacRecord.asset_href(record, band)) as ds:
                    ...
                    # above hangs in docker containers
                    # https://github.com/rasterio/rasterio/issues/2992
                    out[band] = ds.read(1, window=window, fill_value=0, masked=True).filled()
                    if first:
                        first = False
                        print(my_href, flush=True)
                    else:
